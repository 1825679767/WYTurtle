...
这份文档记录 3.3.5 Eluna / mod-eluna 功能移植到本 Turtle/MaNGOS 1.12
源码后的当前状态。
...
```text
<source_dir>\src\game\LuaEngine\TurtleLuaEngine.h
<source_dir>\src\game\LuaEngine\TurtleLuaEngine.cpp
```

Lua 5.2 构建目录：

```text
<source_dir>\dep\lualib\lua
```
...
2. 把 Lua 文件放进运行目录的 `lua_scripts`，或放进 `Eluna.ScriptPath` 指定的目录。
3. 启动 `mangosd.exe`。
4. 登录游戏并输入 `#lua`。
