...
- Player 版本兼容占位：`CanTitanGrip`、成就系统、竞技场点数、雕文、双天赋/专精、职责专精判断、冠军战袍阵营、玩家设置、额外天赋点、`SendShowMailBox`、招募、3.3.5 随机地下城、`BindToInstance` 等接口在 Turtle 1.12 里没有对应 3.3.5 数据结构或安全直接入口，因此当前注册为返回 `0` / `false` / `nil` 或 no-op。这样旧 Eluna 脚本至少不会因为函数不存在而报错；后续如果确认 Turtle 有对应自定义系统，再把占位改成真实实现。
